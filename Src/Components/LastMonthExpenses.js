...
import {
  listWeekExpenses, getStartEndDate, thisMonthExpensesList, lastMonthExpensesList, emptyThisMonthExpenses, emptyLastMonthExpenses
...
class LastMonthExpenses extends Component {
  constructor(props) {
    super(props);
    this.state = {
    	text: "",
  ...
  /* Retrieving top level expenses after this current month, currently not in use */
  retrieveFirstExpenses() {
      this.setState({
          show: true
      })
    firestore().collection("Users").doc(this.props.userToken).collection("DailyExpenses").orderBy("expense_date", "desc").startAt(date).limit(10).get().then(res=>{
        this.props.emptyLastMonthExpenses();
        this.props.lastMonthExpensesList(res);
        var animate_width=[];
  ...
  /* Retrieve more expenses, currently not in use */
  retrieveMoreExpenses() {
    const doc_key = this.props.last_month_expenses[this.props.last_month_expenses.length - 1].expense_date.toDate();
    console.log("Ast Mont", doc_key);
    this.setState({
        refreshing: true
    })
    firestore().collection("Users").doc(this.props.userToken).collection("DailyExpenses").orderBy("expense_date", "desc").startAfter(doc_key).limit(10).get().then(res=>{
        this.props.lastMonthExpensesList(res);
  ...
  /* Retrieving all the expenses at once, it will be depricated after new big release */
  retrieveAllExpenses() {
    this.setState({
      show: true
  })
firestore().collection("Users").doc(this.props.userToken).collection("DailyExpenses").orderBy("expense_date", "desc").startAt(date).onSnapshot(res=>{
    this.props.emptyLastMonthExpenses();
    this.props.lastMonthExpensesList(res);
    var animate_width=[];
    res.forEach((sin, index)=>{
  ...
  render() {
    ...
    return (
      ...
      <View style={PhoneLoginStyles.phoneView} key={this.state.unique_item}>
        ...
        :
        <FlatList
            data={this.props.last_month_expenses}
    ...
    />
            ...
            <Text style={{alignSelf:'center', marginTop:20, fontFamily:'Montserrat-Bold', fontSize:25, color:'lightgrey'}}>NO ADDED EXPENSES</Text>
...
const WrappedComponent = connect(({signInReducer, userDashboardDataReducer, addExpenseReducer, activityReducer}) => {
    ...
    return {
      ...
      emptyLastMonthExpenses: () => dispatch(emptyLastMonthExpenses())
  }})(LastMonthExpenses);
